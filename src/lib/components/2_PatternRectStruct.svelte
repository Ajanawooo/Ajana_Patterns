...
// $inspect(verschiebungY);
	...
	let startColor1 = chroma.oklch(0.8, 0.2, 60); // gelb 
	let color1 = $derived(chroma.scale([startColor1, 'violet']).mode('oklch').domain([200, 110])(verschiebungY).hex());
		...
		{#each Array(11) as _, j}
			...
			<g transform="translate({verschiebungX * (j-0)}, {50 + (j-5) * verschiebungY})">
				{#each Array(20) as _, i}
					<polygon transform="translate({(i-10) * 150}, 0)" points="0 0, -50 0, 0 -100" fill="hotpink" />
					<polygon
